...
from JiuaiSport.settings import REGEX_MOBILE
...
class UserDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ("nickName", "gender", "birthday", "email", "province", "city", "mobile", "openId")


class UserMobileBindingSerializer(serializers.ModelSerializer):
    """
    绑定手机
    """
    ...
    mobile = serializers.CharField(label="电话号码", help_text="电话号码", required=True, allow_blank=False,
                                   validators=[UniqueValidator(queryset=User.objects.all(), message="已经绑定手机")])

    def validate_code(self, code):
        # 用户绑定手机，已post方式提交信息，post的数据都保存在initial_data里面
        ...
        if verify_records:
            ...
            # 有效期为5分钟
            five_minutes_ago = datetime.now() - timedelta(hours=24, minutes=0, seconds=0)
...
class UserMobileRegSerializer(serializers.ModelSerializer):
    '''绑定手机'''
    # UserProfile 中没有code 字段，这里需要自定义一个code序列化字段
    code = serializers.CharField(required=True, write_only=True, max_length=4, min_length=4,
                                 error_messages={
                                        "blank": "请输入验证码",
                                        "required": "请输入验证码",
...
class WXUserRegSerializer(serializers.ModelSerializer):
    """微信用户注册"""
    # 验证用户名是否存在
    openId = serializers.CharField(label="微信openid", help_text="微信openid", required=True, allow_blank=False,
                                   validators=[UniqueValidator(queryset=User.objects.all(), message="用户已经存在")])
